...
def start(message):
    ...
    msg = bot.send_message(message.chat.id, f'Hello, {message.from_user.first_name}!', reply_markup=murkup)
...
def callback_message(callback):
     ...
     if callback.data == 'add':
        ...
        db.commit()
        # msg = bot.message_handler(callback.message.chat.id, 'Write eng words and translate(eng word - translate)')
...
def add_word(message):
    ...
    print(message.text)
    bot.send_message(message.chat.id, f'Its your words: {message.text}')

# @bot.message_handler(func= lambda message: message.text == 'Add new words pare')
# def get_word(message):
    # word_markup = types.InlineKeyboardMarkup()
    # new_words = types.InlineKeyboardButton('Add words', callback_data='new_words')
    # word_markup.add(new_words)
    # bot.send_message(message.chat.id, 'Write eng words and translate(eng word - translate)')
